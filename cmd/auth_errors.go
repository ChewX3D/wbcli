...
func mapAuthError(err error) error {
	...
	switch {
	case errors.Is(err, domainauth.ErrProfileNameRequired):
		return errors.New("--profile is required")
	case errors.Is(err, domainauth.ErrProfileNameInvalid):
		return errors.New("--profile must match [a-zA-Z0-9._-] with length 1..64")
	...
	case errors.Is(err, authservice.ErrCredentialAlreadyExists):
		return errors.New("credentials already exist for this profile; rerun with --force to overwrite")
	case errors.Is(err, authservice.ErrProfileNotFound):
		return errors.New("profile not found")
	case errors.Is(err, authservice.ErrNoActiveProfile):
	...
	case errors.Is(err, ports.ErrCredentialNotFound):
		return errors.New("credentials not found for profile")
